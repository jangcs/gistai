...
if __name__=='__main__':
    url = 'http://localhost:8090/'  # Replace with your API URL
    
    ## Load Detection
    data = json.dumps({'api_name': 'uoais_detection', 'device': 'cuda:0'})
    response = requests.post(url+'load_model', data=data, headers={'Content-Type': 'application/json'})
    print(response.json())
    ...
    response = requests.post(url+'upload', data=data, headers={'Content-Type': 'application/json'})
    print(response.json())
