...
docker run --privileged -it --rm --net host --ipc host --name camera \
...
docker run -it --rm --net host --ipc host --name foodListBuilder asia-northeast3-docker.pkg.dev/cloudrobotai/crms-demo/foodlistbuilder:crms
docker run -it --rm --net host --ipc host --name foodListBuilder \
...
docker run --rm -it --net host --ipc host --name foodMention \
...
## Classification_AI
```
cd ~/crms_demo_robot/classification
...
docker run -it --rm -v /home/jangcs/crms_demo/food_classification:/home/model_store/food_classification \
            --net host --gpus all \
            asia-northeast3-docker.pkg.dev/cloudrobotai/crms-demo/classification:crms
```
## Detection_AI
```
cd ~/crms_demo_robot/detection
...
docker run -it --rm --net host --gpus all asia-northeast3-docker.pkg.dev/cloudrobotai/crms-demo/detection:crms
```
