...
@app.route('/proto_extraction', methods=['POST'])
def proto_extraction():
    global DATA_STORAGE, MODEL, TRANSFORM, DEVICE, API_NAME
    folder_list = json.loads(request.data)['data_list']
    
    spec = importlib.util.spec_from_file_location("module.name", os.path.join(MODEL_STORAGE, API_NAME, 'net.py'))
    foo = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(foo)
    
    for d_folder in folder_list:
        dataset_path = os.path.join(DATA_STORAGE, d_folder)
        foo.extract_prototype(MODEL, TRANSFORM, dataset_path, DEVICE)
